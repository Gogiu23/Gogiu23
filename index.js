...
async function getData(url){
	...
	var config = {
		...
		url: url,
		headers: { 
			'Authorization': process.env.Gogiu23
		}
	};
...
(async()=>{
	const views = await getData('https://api.github.com/repos/Gogiu23/traffic/views');
	const clone = await getData('https://api.github.com/repos/Gogiu23/traffic/clones');
